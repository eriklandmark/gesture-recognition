...
def get_model_v2(num_labels=27, input_shape=(8, 100, 176, 3), learning_rate=0.01):
    model = tf.keras.Sequential()
    model.add(tf.keras.layers.InputLayer(input_shape=input_shape, dtype=tf.float32))
    model.add(tf.keras.layers.Conv3D(filters=32,
                                     kernel_size=(5, 5, 5),
    ...
    model.add(tf.keras.layers.Conv3D(filters=64,
                                     ...
                                     activation="relu"))
    model.add(tf.keras.layers.MaxPooling3D(pool_size=(3, 3, 3), padding="same"))
    model.add(tf.keras.layers.ConvLSTM2D(filters=64, kernel_size=(3, 3), activation="relu"))
    model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))
    model.add(tf.keras.layers.Flatten())
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dense(num_labels, activation="softmax"))

    # model = tf.keras.utils.multi_gpu_model(model, gpus=2, cpu_relocation=True)
...
def get_model_v3(num_labels=27, input_shape=(16, 100, 176, 3), learning_rate=0.01):
    model = tf.keras.Sequential()
    model.add(tf.keras.layers.Conv3D(input_shape=input_shape,
                                     filters=32,
                                     kernel_size=(5, 5, 5),
                                     strides=(1, 2, 2),
                                     dilation_rate=(1, 1, 1),
